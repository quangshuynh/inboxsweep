...
unsubscribe, no background retry, and no Gmail filter or block. Every unsubscribe in this app
begins with a person reading a destination and pressing a button about it, every time.

A [sender rule](Rules.md) does not change that. A rule archives; it has no unsubscribe action to
perform, and a rule for a sender is never treated as permission to contact them.
...
- `List-Unsubscribe-Post` (RFC 8058), which marks the HTTPS entry as a one-click endpoint
  when it carries the exact value `List-Unsubscribe=One-Click`.
